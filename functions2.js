...
const saveSettings = (key, value) => {
    let SettingsData = fs.existsSync(path) ? fs.readFileSync(path, 'utf8').split('\n') : [];
    let found = false;

    settingsData = settingsData.map(line => {
    ...
    if (!found) settingsData.push(`${key}=${value}`);

    fs.writeFileSync(path, settingsData.join('\n'), 'utf8');

    // Reload updated environment variables
    require('dotenv').settings({ path });
};
