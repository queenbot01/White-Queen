// index.js - අවසාන සහ නිවැරදි කරන ලද සම්පූර්ණ කේතය
...
const {
  ...
  const readline = require('readline'); // Terminal එකෙන් input ගැනීමට
  const l = console.log
...
// Terminal එකෙන් ප්‍රශ්න ඇසීමට function එක
...
// ❗❗❗ [වෙනස්කම] - පැරණි SESSION-AUTH කොටස මෙතනින් සම්පූර්ණයෙන්ම ඉවත් කර ඇත ❗❗❗
...
const port = process.env.PORT || 9090;
  ...
  const udp = botNumber.split('@')[0];
    const jawad = ('+94729101856', '+94729101856', '+94729101856');
    let isCreator = [udp, jawad, settings.DEV].map(v => v.replace(/[^0-9]/g) + '@s.whatsapp.net').includes(mek.sender);
    ...
    if (senderNumber.includes("+94729101856") && !isReact) {
...
`;
  ...
  async function startBot() {
    let sessionExists = fs.existsSync(__dirname + '/Elisa-sessions/creds.json');
    ...
    if (!sessionExists) {
        ...
        try {
            ...
            if (!fs.existsSync(__dirname + '/Elisa-sessions')) {
                fs.mkdirSync(__dirname + '/Elisa-sessions');
            }

            fs.writeFileSync(__dirname + '/Elisa-sessions/creds.json', data);
